...
На данный момент объект "extra" используется для передачи данных о клиенте партнёра в направлениях по приёму через шлюзы Qiwi (требование платёжной системы для борьбы с несанкционированными платежами), однако в будущем данный объект может быть расширен любыми данными. Текущий вид данного объекта:
```js
{
    "extra": { 
        "client": {
...
|:--|:--|:--|
| **Шлюз QIWI**                             | invoice | (url) Ссылка для переадресации клиента на выставленный счёт |
| **Шлюз QIWI**                             | direct  | (url) Ссылка для переадресации клиента на страницу прямой оплаты на кошелёк |
